...
function run_queries_testbot($args) {
  ...
  $result = db_query("SELECT MONTH(FROM_UNIXTIME(last_received)), COUNT(test_id) FROM {pifr_test} WHERE type IN (2,3) AND status = 4 AND YEAR(FROM_UNIXTIME(last_received)) = 2014 GROUP BY MONTH(FROM_UNIXTIME(last_received))");
