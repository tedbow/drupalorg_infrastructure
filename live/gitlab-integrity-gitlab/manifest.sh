...
# Projects.
sudo gitlab-psql -d gitlabhq_production -c "COPY (SELECT p.name, p.id, lower(p.path), p.description, n.path FROM projects p LEFT JOIN project_mirror_data pmd ON pmd.project_id = p.id LEFT JOIN namespaces n ON n.id = p.namespace_id) TO STDOUT" | sort > projects.tsv
