...
# Projects.
sudo gitlab-psql -d gitlabhq_production -c "COPY (SELECT p.name, p.id, lower(p.path), p.description, n.path, p.request_access_enabled, s.active, s.title FROM projects p LEFT JOIN namespaces n ON n.id = p.namespace_id LEFT JOIN services s ON s.project_id = p.id AND s.type = 'CustomIssueTrackerService' WHERE p.marked_for_deletion_by_user_id IS NULL) TO STDOUT" | sort > projects.tsv
