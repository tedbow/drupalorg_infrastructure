...
# Projects.
sudo gitlab-psql -d gitlabhq_production -c "COPY (SELECT p.name, p.id, lower(p.path), p.description, n.path FROM projects p LEFT JOIN namespaces n ON n.id = p.namespace_id WHERE p.marked_for_deletion_by_user_id IS NULL) TO STDOUT" | sort > projects.tsv
