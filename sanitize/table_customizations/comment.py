    ...
    def get_sql(self, column_names):
        ...
        if self.dataset == 'skeleton':
            query = """
              ...
              WHERE {source}.{table}.status = 1 AND {source}.node.status = 1 AND ({source}.node.type NOT IN ('project_issue','forum') OR {source}.node.created >= (unix_timestamp() - 60*24*60*60))
              {limit}
            """.format(table=self.table, dest=self.dst, source=self.src, columns=columns, srccolumns=srccolumns, limit=self.limit)
        return query
