    ...
    def get_sql(self, column_names):
        ...
        if self.dataset == 'skeleton':
            query = """
              ...
              SELECT {srccolumns}
              FROM {source}.{table}
              INNER JOIN {source}.node ON {source}.node.nid = {table}.nid AND {source}.node.vid = {table}.vid AND {source}.node.status = 1
              WHERE {source}.{table}.timestamp >= (unix_timestamp() - 60*24*60*60) {limit}
            """.format(table=self.table, dest=self.dst, source=self.src, columns=columns, srccolumns=srccolumns, limit=self.limit)
        return query
