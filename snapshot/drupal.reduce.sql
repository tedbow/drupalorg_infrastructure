...
DELETE node_revisions FROM node_revisions LEFT JOIN node ON node.nid = node_revisions.nid AND node.vid = node_revisions.vid WHERE node.nid IS NULL AND node_revisions.timestamp < (unix_timestamp() - 60*24*60*60);
