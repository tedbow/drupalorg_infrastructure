...
delete_old_ebs_snapshots() {
  ...
  describe_snapshots | grep "${descriptor}" | grep completed | while read i ; do
    # check if the date is of the snapshot is older/smaller than $date7 and delete if it is.
    [[ $(echo ${i} | awk '{print $9}' | awk -F_ '{print $1}') -lt ${date7} ]] && delete_ebs_snapshot $(echo ${i} | awk '{print $2}'); done
}
